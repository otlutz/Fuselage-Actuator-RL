...
class Agent(nn.Module):
    def __init__(self, envs, n_actions):
        ...
        self.actor_logstd = nn.Parameter(args.actor_logstd*torch.ones(1, np.prod(envs.single_action_space.shape)))  # initial action_std = exp(actor_logstd)
...
if __name__ == "__main__":
    ...
    envs = gym.vector.AsyncVectorEnv(
        [make_env(args.env_id, args.seed + i, i, args.capture_video, run_name, args.n_actions, args.mode, args.record) for i in range(args.num_envs)]
    )
    assert isinstance(envs.single_action_space, gym.spaces.Box), "only continuous action space is supported"

    agent = Agent(envs, args.n_actions).to(device)
    ...
    if args.track and wandb.run.resumed:
        starting_update = run.summary.get("update") + 1
    ...
    envs = gym.vector.AsyncVectorEnv(
        [make_env(args.env_id, args.seed + i, i, args.capture_video, run_name, args.n_actions, "Test", args.record) for i in range(1)]
    )
